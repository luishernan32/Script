--Punto 3
--ACTA_DE_POSESION_P  2640 10118
--order variables: numero_consecutivo,FECHA_GENERACION,UBICACION_SEDE,NOMBRE_DELEGADO,NOMBRE_ABOGADO_ASIG,NOMBRE_ABOGADO,NUMERO_MEMORANDO,FECHA_MEMORANDO,FECHA_SOLICITUD,DOCUMENTO_PARTICIPANTE,T_DOCUMENTO_PARTICIPANTE,IMAGEN_FIRMA,DELEGADO_NOMBRAMIENTO_,DELEGADO_TITULONOMBRE_,IMAGEN_FIRMA_DOS,ESTADO,CARGO_DELEGADO
...
------------------------------------------------------------------------------------------
--Consulta modificada para mostrar datos

DECLARE @FECHA DATE



SELECT top 1 c.numero_coactivo,
       tp.fecha_inicio                                             AS fecha1,
       'Centro Comercial Albán Borja, local 56 A y B'             AS direccion,
       (SELECT Concat (Substring(f.titulo_obtenido, 1, 1), Lower(
                       Substring(f.titulo_obtenido, 2, Len(f.titulo_obtenido))),
