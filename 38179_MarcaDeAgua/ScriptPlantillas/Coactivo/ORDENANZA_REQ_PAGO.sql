...
SELECT 
  ...
  TOP 1 cp.id_factura_axis numero_factura, 

  CASE p.id_tipo_identificacion WHEN 2 THEN Upper (pj.nombre_comercial) ELSE Upper(
  ...
  p.numero_identificacion documentoInfractor, 
    ...
    (
      ...
      SELECT 
        ...
        TOP 1 Cast(
          ...
          u.direccion_de_entrega AS VARCHAR(200)

        ) 

      FROM 

        integracion_terceros..ubicabilidad_courier_validada u 

      WHERE 

        u.id_factura_axis = cp.id_factura_axis

    ), 

    ''''

  ) AS direccionInfractor, 

  cp.numero_citacion numeroCitacion, 

  (

    i.codigo_infraccion + '' - '' + ci.descripcion
  ...
  (
    ...
    WHERE 
      ...
      cp.id_direccion_comparendo = d.id_direccion
  ...
  Concat(
    ...
    (
      ...
      WHERE 
        ...
        id_normatividad = ci.id_normatividad
  ...
  (
    ...
    FROM 
      ...
      comparendo_cartera cc (nolock) 

      JOIN cartera ca (nolock) ON cc.id_cartera = ca.id_cartera 

    WHERE 

      cc.cicomparendo = cp.cicomparendo
  ...
  CONVERT(
    ...
    dt.fecha_calibracion, 

    103

  ) fechaCalibracion, 

  dt.velocidad_via velocidadPermitida, 

  dt.velocidad_infraccion velocidadInfraccion, 
  ...
  (
    ...
    SELECT 
      ...
      upper(
        ...
        SUBSTRING(valor_parametro_defecto, 18, 62)
    ...
    WHERE 
      ...
      codigo_parametro = 434

  ) AS CargoDelegado, 

  Concat(

    pa.nombre1, '' '', pa.nombre2, '' '', pa.apellido1, 
...
FROM 
  ...
  comparendo (nolock) cp 

  INNER JOIN comparendo_persona (nolock) cper ON cper.cicomparendo = cp.cicomparendo 

  INNER JOIN persona (nolock) p ON p.id_persona = cper.id_persona 

  INNER JOIN tipo_identificacion_persona tipoid ON tipoid.id_tipo_identificacion = p.id_tipo_identificacion 

  LEFT JOIN persona_juridica (nolock) pj ON pj.id_persona_juridica = p.id_persona 

  LEFT JOIN configuracion_infraccion (nolock) ci ON cp.id_infraccion = ci.id_infraccion 

  AND ci.fecha_fin_vigencia IS NULL 

  INNER JOIN infraccion (nolock) i ON i.id_infraccion = ci.id_infraccion 

  LEFT JOIN comparendo_vehiculo (nolock) cpvh ON cpvh.id_comparendo_vehiculo = cp.cicomparendo 
  ...
  LEFT JOIN (
    ...
    FROM 
      ...
      integracion_terceros..v_datos_infracciones (nolock)

  ) dt ON dt.id_factura = cp.id_factura_axis 

  LEFT JOIN comparendo_agente (nolock) ca ON ca.cicomparendo = cp.cicomparendo 

  LEFT JOIN agente ag ON ca.id_agente = ag.id_agente 

  LEFT JOIN persona pa ON pa.id_persona = ag.id_persona 

  LEFT JOIN firma_persona fa ON fa.id_persona = pa.id_persona 

  LEFT JOIN ordenamiento_pais ord ON ci.id_ordenamiento_pais = ord.id_ordenamiento_pais 

WHERE 

  cp.id_factura_axis = : numeroObligacion
