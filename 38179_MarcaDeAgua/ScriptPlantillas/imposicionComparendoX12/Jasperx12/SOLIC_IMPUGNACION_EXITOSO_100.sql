...
(SELECT valor_parametro_defecto
 ...
 WHERE  codigo_parametro = 439)                                            AS
       TITULO_PLANTILLAS,
(SELECT ( Concat (f.memo_nombramiento, ', de fecha  ', f.fecha_nombramiento) )
 FROM   usuario_persona up
        JOIN funcionario f
          ON f.id_persona = up.id_persona
 WHERE  f.id_persona = delegado.id_persona)                                AS
       fecha_nombramiento
FROM   (SELECT pr.id_proceso,
       ...
       (SELECT TOP 1 fa.numero_imagen        firmaAgente,
                     fu.id_persona,
                     Upper(Concat (fu.titulo_obtenido, ' ', nombre1, ' ',
                           nombre2, ' ',
                           apellido1,
                           ' ',
                                 apellido2)) AS nombre
