...
SET    consulta=   'SELECT p.numero_proceso,
       ...
       (SELECT TOP 1 Concat(per.nombre1, '' '', per.nombre2, '' '', per.apellido1,
		...
		and tp.id_estado_proceso=139 ) AS nombreAbogadoFirma,
...
WHERE  p.id_proceso = :idProceso
	   ...
	   , orden_variables='numero_consecutivo,NOMBRE_COMPLETO,CORREO_ELECTRONICO_INFRACTOR,anio_apertura_proceso,NOMBRE_ABOGADO,PLACA_VEHICULO,NOMBRE_DELEGADO,  CARGO_DELEGADO,IMAGEN_FIRMA,MEMO_DELEGADO,FECHA_DELEGADO,TEXTO_NOMBRE_2'
       ...
       (SELECT Upper(Concat(PER.nombre1, ' ', per.nombre2, ' ', per.apellido1,
        ...
        FROM   trazabilidad_proceso tp (nolock)
               ...
               JOIN persona per (nolock)
                 ON up.id_persona = per.id_persona
        WHERE  tp.id_estado_proceso = 6
               AND tp.id_proceso = 2833597)   AS nombreUsuario,
       cv.placa_vehiculo,
...
FROM   proceso p (nolock)
       ...
       JOIN persona per (nolock)
         ON per.id_persona = up.id_persona
		 --JOIN acta_resorteo ar(nolock)
		--ON  fu.id_funcionario=ar.id_funcionario
       JOIN funcionario fu (nolock)
         ON fu.id_funcionario = (SELECT id_funcionario
                                 FROM   evaluar_impugnacion (nolock)
                                 WHERE  id_trazabilidad_proceso =
...
WHERE  p.id_proceso = 2833597
       AND co.id_estado_comparendo != 14
       AND tp.id_estado_proceso = 10
       AND pp.id_tipo_participante = 2 
	   --AND fu.id_funcionario=ar.id_funcionario
